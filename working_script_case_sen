...
# Extract patterns from RELEASE_NOTES.md
branch_patterns=($(grep -o 'bias-[0-9]\+' "$RELEASE_NOTES_FILE" | sort | uniq))
...
# Find branches in the remote repository that match the patterns
matching_branches=()
for pattern in "${branch_patterns[@]}"; do
    for remote_branch in $(git branch -r | grep "$pattern"); do
